...
import (
	...
	"github.com/pocc/go-pocc/trie"
)

const defaultTraceTimeout = 5 * time.Second

// PublicPoccAPI provides an API to access Pocc full node-related
// information.
type PublicPoccAPI struct {
	e *Pocc
}

// NewPublicPoccAPI creates a new Pocc protocol API for full nodes.
func NewPublicPoccAPI(e *Pocc) *PublicPoccAPI {
	return &PublicPoccAPI{e}
}

// Poccbase is the address that mining rewards will be send to
func (api *PublicPoccAPI) Poccbase() (common.Address, error) {
	return api.e.Poccbase()
}

// Coinbase is the address that mining rewards will be send to (alias for Poccbase)
func (api *PublicPoccAPI) Coinbase() (common.Address, error) {
	return api.Poccbase()
}

// Hashrate returns the POW hashrate
func (api *PublicPoccAPI) Hashrate() hexutil.Uint64 {
...
func NewPublicMinerAPI(e *Pocc) *PublicMinerAPI {
...
func NewPrivateMinerAPI(e *Pocc) *PrivateMinerAPI {
...
// SetPoccbase sets the poccbase of the miner
func (api *PrivateMinerAPI) SetPoccbase(poccbase common.Address) bool {
	api.e.SetPoccbase(poccbase)
	return true
}
...
// PrivateAdminAPI is the collection of Pocc full node-related APIs
...
// admin methods of the Pocc service.
func NewPrivateAdminAPI(eth *Pocc) *PrivateAdminAPI {
...
// PublicDebugAPI is the collection of Pocc full node APIs exposed
...
// related public debug methods of the Pocc service.
func NewPublicDebugAPI(eth *Pocc) *PublicDebugAPI {
...
// PrivateDebugAPI is the collection of Pocc full node APIs exposed over
...
// private debug methods of the Pocc service.
func NewPrivateDebugAPI(config *params.ChainConfig, eth *Pocc) *PrivateDebugAPI {
...
type BlockTraceResult struct {
	...
	StructLogs []poccapi.StructLogRes `json:"structLogs"`
...
func (api *PrivateDebugAPI) TraceBlock(blockRlp []byte, config *vm.LogConfig) BlockTraceResult {
	...
	return BlockTraceResult{
		Validated:  validated,
		StructLogs: poccapi.FormatLogs(logs),
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, txHash common.Hash, config *TraceArgs) (interface{}, error) {
	...
	if config != nil && config.Tracer != nil {
		...
		var err error
		if tracer, err = poccapi.NewJavascriptTracer(*config.Tracer); err != nil {
			return nil, err
		}
		...
		go func() {
			<-deadlineCtx.Done()
			tracer.(*poccapi.JavascriptTracer).Stop(&timeoutError{})
		}()
		defer cancel()
	} else if config == nil {
	...
	case *vm.StructLogger:
		return &poccapi.ExecutionResult{
			Gas:         gas,
			Failed:      failed,
			ReturnValue: fmt.Sprintf("%x", ret),
			StructLogs:  poccapi.FormatLogs(tracer.StructLogs()),
		}, nil
	case *poccapi.JavascriptTracer:
