...
// EthApiBackend implements poccapi.Backend for full nodes
...
func (b *EthApiBackend) GetPocBlocks(ctx context.Context, blockPage rpc.BlockNumber, blockCnt uint64) ([]*types.BlockSummary, error) {
	ch := b.eth.blockchain.CurrentBlock().Header()
	...
	for i := ch.Number.Uint64() - uint64(blockPage - 1) * blockCnt; i + uint64(blockPage) * blockCnt > ch.Number.Uint64() && i >= uint64(0); i-- {
		bl := b.eth.blockchain.GetBlockByNumber(i)
		if bl != nil {
