...
func TestAccountListEmpty(t *testing.T) {
	pocc := runPocc(t, "account", "list")
	pocc.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	pocc := runPocc(t, "account", "list", "--datadir", datadir)
	defer pocc.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	pocc := runPocc(t, "account", "new", "--lightkdf")
	defer pocc.ExpectExit()
	pocc.Expect(`
...
`)
	pocc.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	pocc := runPocc(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer pocc.ExpectExit()
	pocc.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(pocc.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(pocc.StderrText(), m) {
...
// https://github.com/pocc/go-pocc/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	pocc.SetTemplateFunc("keypath", func(file string) string {
