...
import (
	...
	"github.com/pocc/go-pocc/accounts/usbwallet/internal/trezor"
...
// or whether the Pocc app was not started on it.
...
// Pocc address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.PoccAddress)
	if _, err := w.trezorExchange(&trezor.PoccGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.PoccTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.PoccTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
