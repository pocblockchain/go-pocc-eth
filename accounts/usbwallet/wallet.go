...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Pocc
...
type wallet struct {
	...
	deriveChain    pocc.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Pocc account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain pocc.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
