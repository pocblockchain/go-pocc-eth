...
func newTester(t *testing.T, confOverride func(*eth.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Pocc service within
	...
	ethConf := &eth.Config{
		...
		Poccbase: common.HexToAddress(testAddress),
		PowTest:   true,
	}
	if confOverride != nil {
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return eth.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register Pocc protocol: %v", err)
	}
