...
import (
	...
	"github.com/pocc/go-pocc/accounts/keystore"
	"github.com/pocc/go-pocc/common"
	"github.com/pocc/go-pocc/common/hexutil"
	"github.com/pocc/go-pocc/common/math"
	...
	"github.com/pocc/go-pocc/crypto"
...
// PublicPoccAPI provides an API to access Pocc related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicPoccAPI struct {
	b Backend
}

// NewPublicPoccAPI creates a new Pocc protocol API.
func NewPublicPoccAPI(b Backend) *PublicPoccAPI {
	return &PublicPoccAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicPoccAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Pocc protocol version this node supports
func (s *PublicPoccAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicPoccAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Pocc Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Pocc Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Pocc ECDSA signature for:
// keccack256("\x19Pocc Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/pocc/go-pocc/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Pocc Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/pocc/go-pocc/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Pocc signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Pocc blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Pocc blockchain API.
...
//
// https://github.com/pocc/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of Pocc APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Pocc APIs exposed over the private
// debugging endpoint.
...
// Version returns the current pocc protocol version.
