...
type Config struct {
	Oracle common.Address // Pocc address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the Pocc service dependency to access the blockchain
	var apiBackend poccapi.Backend
	var pocc *eth.Pocc
	if err := ctx.Service(&pocc); err == nil {
		apiBackend = pocc.ApiBackend
	} else {
		var pocc *les.LightPocc
		if err := ctx.Service(&pocc); err == nil {
			apiBackend = pocc.ApiBackend
		} else {
			return nil, err
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/pocc/go-pocc/releases for new releases")
